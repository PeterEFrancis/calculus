...
% Text Node
\draw (293,225) node [anchor=north west][inner sep=0.75pt]    {$a$};
% Text Node
...
The inverse rule requires $f$ to be differentiable and one-to-one on an
interval, with $f'(f^{-1}(x))\ne0$ at the point in question.
...
\quad\quad\quad\quad\quad
[\csc(x)]' = -\csc(x)\cot(x)$$
\mar{Try to find a pattern with these four derivatives.}
...
\begin{remark}
Over the real numbers, this method assumes that $f(x)>0$ on the interval under consideration. 
% Thus $x^x$ is treated on $x>0$, while $\sin(x)^x$ is treated only on intervals where $\sin(x)>0$. A negative base with a variable real exponent is generally not real-valued.
\end{remark}
...
An alternate method for logarithmic differentiation is to transform $f(x)^{g(x)}$ into a form more easily differentiable by first taking the logarithm and then exponentiating:
$$f(x)^{g(x)}=e^{\ln\left(f(x)^{g(x)}\right)}=e^{g(x)\ln(f(x))}.$$
\mar{Take the derivative of $e^{g(x)\ln(f(x))}$ and see that it matches the expression for $y'$ in the strategy box.}
\subsection{Using The Inverse Rule}

Let $f$ be differentiable and injective (one-to-one) on an interval $I$, and let $x$ be an
interior point of $f(I)$. If $f'(f^{-1}(x))\ne0$, then $f^{-1}$ is
differentiable at $x$ and the inverse rule says that
$$[f^{-1}(x)]'=\frac{1}{f'(f^{-1}(x))}.$$
% \mar{Why is the nonzero condition necessary? For $f(x)=x^3$, $f'(0)=0$, and $f^{-1}(x)=\sqrt[3]{x}$ has no finite derivative at $0$.}
It is easy to see where this formula comes from:
\begin{align*}
         & f(f^{-1}(x)) = x & & \text{(definition of inverse)}\\
\implies & f'(f^{-1}(x))[f^{-1}(x)]'=1 & &\text{(chain rule)}\\
\implies & [f^{-1}(x)]' =\frac{1}{f'(f^{-1}(x))} & & \text{(solve for $[f^{-1}(x)]'$)}.
\end{align*}

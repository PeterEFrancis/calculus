...
\textbf{Exponential Rules} & $\int a^x\ dx = \frac{1}{\ln(a)}a^x+C$, where $a>0$ and $a\ne1$ \\
\bottomrule[0.4mm]
\end{tabular}
\end{center}
...
\bottomrule[0.4mm]
    \end{tabular}
\end{center}
\mar{Do you see the pattern? What terms would you add for a quartic factor in the denominator?}
...
\noindent\textbf{Assumptions.} Assume $a>0$. For $x=a\sin\theta$, take
$-\pi/2\le\theta\le\pi/2$; for $x=a\tan\theta$, take
$-\pi/2<\theta<\pi/2$. For $x=a\sec\theta$, take
$0\le\theta<\pi/2$ when $x\ge a$, or $\pi\le\theta<3\pi/2$ when $x\le-a$.
These choices make each substitution one-to-one and justify removing the
absolute values from the square-root simplifications.

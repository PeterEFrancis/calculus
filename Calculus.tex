...
It turns out that a function is invertible \underline{exactly} when it is bijective. If a function is not bijective, we can ``fix'' it by making its domain smaller.
...
\item The function $\sin$ has domain $\R$ and range $[-1,1]$ but is \textbf{periodic} (meaning its values cyclicly repeat) so cannot be invertible without an adjustment to its domain. The 
\end{itemize}

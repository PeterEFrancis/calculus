...
\begin{thm}
Let $f$ and $g$ be differentiable on a deleted neighborhood of $a$ (or on a
tail interval when $a=\pm\infty$), with $g(x)\ne0$ and $g'(x)\ne0$ there.
Suppose that $f/g$ has the indeterminate form $0/0$ or $\infty/\infty$ as
$x\to a$. If
$$\lim_{x\to a}\frac{f'(x)}{g'(x)}=L$$
exists, possibly with $L=\pm\infty$, then
$$\lim_{x\to a}\frac{f(x)}{g(x)}=L.$$
\end{thm}
...
Sometimes you will encounter an indeterminate form that is not $\frac00$ or $\frac\infty\infty$, and in these cases, we can transform it into the form $\frac00$ or $\frac\infty\infty$ and then use L'H\^optial's rule. Here are some illustriative examples:
...
\mar{Label each of these examples with original and trasnsformed type of indeterminate forms they demonstrate.}
...
\subsection{Extrema and Critical Numbers}

\noindent A function $f$ has a \textbf{local maximum} at $a$ if there is an open interval $I\ni a$ such that
$$f(a)\geq f(x)\qquad\text{for every }x\in I\cap\operatorname{dom}(f).$$
A \textbf{local minimum} is defined in the same way, with $f(a)\leq f(x)$.

The function has a \textbf{global maximum} at $a$ if
$$f(a)\geq f(x)\qquad\text{for every }x\text{ in the domain of }f.$$
A \textbf{global minimum} is defined in the same way, with $f(a)\leq f(x)$.
When one of these conditions holds, $a$ is a location of an extremum, $f(a)$ is the corresponding \textbf{extremal value}, and $(a,f(a))$ is the corresponding \textbf{extremal point} on the graph of $f$.

A global maximum or minimum value, if it exists, is unique, but it may occur at more than one input. For example, $\sin(x)$ has a global maximum value of $1$, even though this value occurs infinitely many times on the domain of $\sin(x)$. Every global extremum is also a local extremum.


A number $a$ in the domain of $f$ is called a \textbf{critical number} of $f$ if $f'(a)=0$ or $f'(a)$ does not exist. The corresponding graph point $(a,f(a))$ is called a \textbf{critical point} of $f$. The following theorem tells us how to find extrema using critical numbers.

\begin{thm}
If $f$ has a local extremum at $a$, then $a$ is a critical number of $f$.
\end{thm}

\noindent WARNING: The converse is not true: not every critical number is the location of a local extremum.\mar{Name and draw a function with a critical number at which it has no local extremum.}

\begin{thm}[First Derivative Test]
Suppose $a$ is a critical number of $f$ and $f$ is continuous at $a$.
Then, sufficiently close to $a$
\begin{itemize}
\item if $f'$ changes from $(+)$ to $(-)$, then $f$ has a local maximum at $a$.
\item if $f'$ changes from $(-)$ to $(+)$, then $f$ has a local minimum at $a$.
\item if $f'$ has the same strict sign on both sides of $a$, then $f$ has no local extremum at $a$.
\end{itemize}
\end{thm}
...
\text{then $f$ has}
\begin{cases}
\text{a local minimum at $a$}\\
\text{a local maximum at $a$}
\end{cases}.$$
\end{thm}


\begin{strat}[Finding Extrema]
\begin{enumerate}[leftmargin=1em]
\item Solve $f'(x)=0$ for $x$.
\item Find any other critical numbers: numbers in the domain at which $f'$ does not exist, such as endpoints, corners, cusps, vertical tangents, and discontinuities at which $f$ is defined.
\item If you are looking for \textit{local} extrema, use the first or second derivative tests where applicable to determine which critical numbers are locations of local extrema.
\item If you are looking for \textit{global} extrema, evaluate $f$ at each critical number and compare the resulting values.
\end{enumerate}
\end{strat}
...
The strategy for optimization problems is very similar to the strategy for related rates, but uses the theory of extrema from the previous section.
...
\item Find the locations and values of the extrema of $A$.
\end{enumerate}
\end{strat}
...
Imagine that you have a function $f$ and you want to find the zeros (or roots) of $f$. That is, you want to find the values of $x$ such that $f(x)=0$. \textbf{Newton's Method} is a computational algorithm that can sometimes be used to approximate such a value. First, we'll describe the algorithm, and then we'll show how to use your TI calculator to implement it.

Imagine that $x_0$ is an initial guess of a root of the function $f$. If $f'(x_0)\ne 0$, then the tangent line of $f$ at $x_0$ is
$$y-f(x_0)=f'(x_0)(x-x_0)$$
and so we find (setting $y=0$ and solving for $x$) that this tangent line intersects the $x$-axis at the point $(x_1,0)$, where
$$x_1=x_0-\frac{f(x_0)}{f'(x_0)}.$$
We continue in the same way. As long as $f'(x_n)\ne0$, set
$$x_{n+1}=x_n-\frac{f(x_n)}{f'(x_n)}.$$
If $f(x_n)=0$, stop: we have found a root. If $f'(x_n)=0$ before we find a root, the method gets stuck and we need a different guess.

In many cases, this sequence of numbers approaches a zero of $f$. The idea is simple: near $x_n$, use the tangent line as an approximation of the graph and take the zero of that line as the next guess. The sign of $-f(x_n)/f'(x_n)$ tells us which way the next guess moves:
\begin{itemize}
\item $x_n<x_{n+1}$ (move to the right) when $f(x_n)$ and $f'(x_n)$ have opposite signs. \mar{Draw a picture showing the four cases.}
\item $x_{n+1}<x_n$ (move to the left) when $f(x_n)$ and $f'(x_n)$ have the same sign.
\end{itemize}

This tells us whether the next guess lies to the left or right, but it does not guarantee that it is closer to a root. Newton's method can move away from a root, enter a cycle, encounter a point where $f'=0$, or converge to a different root. However, if $r$ is a root with $f'(r)\ne0$, $f$ is smooth near $r$, and $x_0$ is close enough to $r$, then Newton's method converges to $r$.
...
Although Newton's method is generally an easy algorithm to implement and use, it does not always work.\mar{Try $f(x)=x^3-2x+2$ with $x_0=0$. Show that the guesses cycle: $0\to1\to0\to\cdots$.}

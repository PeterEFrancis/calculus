...
\section*{P.P.S}
Ok... I lied. The pebbles actually refer to the beads on an abacus, but a professor of mine once told me the small pieces version, and I like that better!
...
In this table, I wrote the numbers in an ``un-simplified'' way so you can see the pattern: their numerators change by 1 under a square-root, and as one goes up, they other goes down.
...
Dividing this equation by $\sin^2\theta$ and $\cos^2\theta$, we get
$$\cot^2\theta + 1 = \csc^2\theta\quad\quad\text{and}\quad\quad 1+\tan^2\theta = \sec^2\theta.$$

Since $\sin\theta$ and $\cos\theta$ are the leg lengths of a right triangle with hypotenuse 1, there is some nice symmetry between the two quantities. The leg opposite the angle is $\theta$ is $\sin\theta$. Since the internal angles of a triangle sum to $\pi$, the other acute angle is $\frac{\pi}{2} - \theta$. \mar{Label the angle $\frac{\pi}{2}-\theta$ on the picture above.} Then, the leg opposite $\frac{\pi}{2} - \theta$ has length $\sin\left(\frac{\pi}{2} - \theta\right)$, but is also $\cos\theta$. Therefore,
...
Let's say you have the following right triangle.
...
\draw   (426,75) -- (252.56,186.5) -- (426,186.5) -- cycle ;
%Shape: Arc [id:dp8301261755391214] 
\draw  [draw opacity=0] (274.77,172.15) .. controls (277.45,176.28) and (279,181.21) .. (279,186.5) -- (252.56,186.5) -- cycle ; \draw   (274.77,172.15) .. controls (277.45,176.28) and (279,181.21) .. (279,186.5) ;  
%Shape: Rectangle [id:dp7819843676440785] 
\draw   (412.88,173.38) -- (426,173.38) -- (426,186.5) -- (412.88,186.5) -- cycle ;

% Text Node
\draw (280.25,170) node [anchor=north west][inner sep=0.75pt]    {$30\degree $};
% Text Node
\draw (323,120) node [anchor=north west][inner sep=0.75pt]    {$8$};
% Text Node
\draw (345,190) node [anchor=north west][inner sep=0.75pt]    {$x$};
% Text Node
\draw (430,130) node [anchor=north west][inner sep=0.75pt]    {$y$};

\end{tikzpicture}

\end{center}

If you want to know what the other side lengths are, this is how you can use trigonometry to do it! First, scale the triangle down by the side length that you know (in this case $8$).
...
\draw (280.25,170) node [anchor=north west][inner sep=0.75pt]    {$30\degree $};
% Text Node
\draw (323,120) node [anchor=north west][inner sep=0.75pt]    {$1$};
% Text Node
\draw (345,190) node [anchor=north west][inner sep=0.75pt]    {$x/8$};
% Text Node
\draw (430,130) node [anchor=north west][inner sep=0.75pt]    {$y/8$};

\end{tikzpicture}

\end{center}

This doesn't change the angles, so we now have a right triangle with hypotenuse 1. By definition of $\sin$ and $\cos$,
...
The above picture shows a triangle with internal angles $A$, $B$, and $C$, with respective opposite side lengths $a$, $b$, and $c$. The triangle is \textbf{circumscribed} in a circle, meaning its vertices lie on the circle's boundary. It turns out that the circle has radius
...
\end{theorem}
\mar{Looks almost familiar, doesn't it?}
...
The set of real numbers ($\R$) is an uncountably infinite set, meaning I can not even begin to exhaustively list all of its members. However, here are some: $3$, $2/5$, $4.11$, $\sqrt{37}$, $\pi$. Real numbers include the rationals and irrationals. \mar{Do you know any numbers that aren't real?}
...
$$\{\underbrace{x}_x\underbrace{\in}_\text{in}\underbrace{\R}_{\text{the real}\atop \text{numbers}}\ \underbrace{:}_\text{such that}\ \underbrace{a < x < b}_{\text{$x$ is strictly}\atop\text{between $a$ and $b$}}\}.$$
...
\mar{Draw a picture of each of these examples. Does Venn diagram ring any bells?}
...
The \textbf{domain} of a function is the set of possible inputs. The \textbf{range} of a function is the set of possible outputs. Not all real functions have $\R$ as their domain and range. The following table domains and ranges for some common functions.
...
Let's do some more concrete examples:
\begin{itemize}
\item The function $f$ given by $f(x)=\frac{1}{x}$ does not have 0 in its domain, since we can't divide by 0. Also, $f$ does not have 0 in its range, since there is no real number $x$ for which $\frac{1}{x}=0$. Therefore the domain and range are both $(-\infty,0)\cap(0,\infty)$.
\item Similarly, $\tan$ does not have $2n\pi+\pi/2$ in its domain for any integer $n$, since $\cos(2n\pi+\pi/2)=0$ (we can write the domain of $\tan$ as $\{x\in\R\ :\ x\neq 2n\pi + \pi/2\text{ for some integer }n\}$).  The domain of $\tan$ is all of $\R$.

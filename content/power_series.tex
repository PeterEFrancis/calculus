...
\end{thm}
\mar{Draw a picture of the interval $[a-d, a+d]$. See how it's the same as $|x-a|<d$?}
...
\end{tabular}
\end{center}
where if $d_1=\sqrt[k+1]{\epsilon\cdot (k+1)!/M}$. \mar{Check the algebra solving for $d_1$.} If we want $|f(x)-T_k(x)|<\epsilon$, $|x-a|$ must be smaller than both $d_0$ and $d_1$. Thus the $d$ we are after is $d=\min\left(d_0, d_1\right)$.

...
\item $\displaystyle\sum_{n=1}^\infty \frac{(-1)^n}{n}(x-1)^n$
...
The function $T_k(x)$ is called the \textbf{degree-$k$ Taylor polynomial} for $f$ at $a$. Note that $f^{(k)}(a)$ denotes the $k$th derivative of $f$ evaluated at $a$. The difference between $f(x)$ and its degree-$k$ Taylor polynomial is called the \textbf{Taylor remainder}:
$$R_k(x)=f(x)-T_k(x).$$
Thus $f(x)=T_k(x)+R_k(x)$. If, for a particular value of $x$,
$$\lim_{k\to\infty}R_k(x)=0,$$
then taking $k\to\infty$ gives
$$f(x)= \sum_{n=0}^\infty\frac{f^{(n)}(a)}{n!}(x-a)^n.$$
The infinite series on the right is called the \textbf{Taylor series} for $f$ at $a$. When $a=0$, the series is called a \textbf{Maclaurin series}.

As a power series centered at $a$, a Taylor series has a radius of convergence $r$: it converges when $|x-a|<r$, diverges when $|x-a|>r$, and its behavior at the endpoints must be checked separately. If $r=0$, the interval of convergence is the singleton set $\{a\}$; if $r=\infty$, the series converges for every real number. Caution: the interval of convergence describes only where the Taylor series converges to something (not necessarily $f(x)$). At a point in that interval, the Taylor series equals $f(x)$ only when the Taylor remainder tends to zero there.
\mar{Consider the function 
$$f(x)=\begin{cases} e^{-1/x^2} & x\neq 0\\ 0 & x=0. \end{cases}$ Determine the Taylor series and interval of convergence of $f$ at $x=0$. Compare the value of series and the function on the IOC.}

The following table lists various common Taylor series.
...
$(x+1)^k$ & $\displaystyle\sum_{n=0}^{k}{k\choose n}x^{n}$ & $\R\ (k\in\mathbb{Z}_{\geq 0})$ \\\\
\bottomrule[0.4mm]
\end{tabular}
\end{center}

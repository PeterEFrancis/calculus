...
A sequence $(a_n)$ is \textbf{bounded} if there is some $B$ for which $-B<a_n<B$ for all $n$. A sequence is \textbf{monotonic} if it is always increasing ($a_n\leq a_{n+1}$ for all $n$)  or decreasing ($a_n\geq a_{n+1}$ for all $n$) .
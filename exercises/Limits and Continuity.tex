...
\item If $f$ and $g$ are continuous at $a$, then $\displaystyle\lim_{x\to a} (f(x) + g(x))= f(a) + g(a).$
...
\begin{enumerate}
\item For which numbers $a$ does $\displaystyle \lim_{x\to a} f(x)=L$ but $L\neq f(a)$?
\item At which numbers is $f$ continuous?
\item At which numbers is $f$ discontinuous?
...
\subsection*{G. Locate and classify the discontinuities of the following functions.}
\begin{multicols}{2}
\begin{enumerate}
\item $f(x)=\begin{cases}
\displaystyle\frac{\sin(3x^2)}{x-1}& 0\leq x\\[1em]
\displaystyle\frac{(x^2+1)(x-2)}{x^2-4} & x < 0
\end{cases}$
\item $f(x)=\begin{cases}
\displaystyle\frac{x-5}{x^{2}-7x+10} & 2<x\\[1em]
\displaystyle\frac{1}{1-2^{\left(x-2\right)}} & -1\leq x <2 \\[1em]
\displaystyle \ln\left(-x\right) & x < -1\end{cases}$
\end{enumerate}
\end{multicols}

\subsection*{H. Use the IVT to show that each equation has a solution on the given interval.}
\begin{multicols}{3}
\begin{enumerate}

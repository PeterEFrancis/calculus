...
These functions also have interesting visual representations on the unit circle.
\begin{figure}[h!]
\centering
\fbox{
\begin{tikzpicture}[x=0.75pt,y=0.75pt,yscale=-1,xscale=1]
...
$$\text{for all } \epsilon > 0, \text{ there is some } \delta > 0, \text{ such that } |x - a| < \delta \implies |f(x) - L| <\varepsilon.$$
...
While the function graphed in the top two plots is continuous at $a$, the function that is graphed in the bottom two plots is not. For this function the first value of $\varepsilon$ (graph on the left) is not small enough to detect the discontinuity, but on the second (graph on the right), $\varepsilon$ is so small that no value of $\delta$ can be small enough.
...
\begin{itemize}
\item $x_{n+1}>x_n$ (move to the right).  Algebraically, this occurs exactly when $f(x_n)$ and $f'(x_n)$ have opposite sign (when $f$ is increasing below the $x$-axis, or decreasing above the $x$-axis).\mar{Draw a picture showing how in both of these sub-cases, the root of $f$ can be to the right of $x_n$.}
\item $x_{n+1}<x_n$ (move to the left). Similarly, this occurs exactly when $f(x_n)$ and $f'(x_n)$ have the same sign (when $f$ is increasing above the $x$-axis, or decreasing below the $x$-axis).\mar{Draw a picture showing how in both of these sub-cases, the root of $f$ can be to the left of $x_n$.}
\end{itemize}

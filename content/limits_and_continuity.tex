...
\begin{tikzpicture}[
    ...
    choice/.style={draw, rounded corners, align=center, minimum height=0.9cm, inner sep=4pt}
]
\node[choice] (plug) at (0,2.2) {plug $\displaystyle a$ into $f$};

\node[choice, minimum width=1.7cm] (number) at (-4.45,0.5) {number};
\node[choice, text width=2.8cm] (overzero) at (-1.65,0.5)
    {$\displaystyle\frac{\text{non-zero number}}{0}$};
\node[choice, text width=2.2cm] (indeterminate) at (1.2,0.5)
    {indeterminate\\form};
\node[choice, text width=2.7cm] (undefined) at (3.95,0.5)
    {undefined for\\another reason};

\node (number-result) at (-4.45,-0.9) {$f(a)$};
\node (overzero-result) at (-1.65,-0.9) {DNE or $\displaystyle\pm\infty$};
\node[align=center] (indeterminate-result) at (1.2,-0.9) {More work\\needed};
\node[align=center] (undefined-result) at (3.95,-0.9) {More work\\needed};

...
\item Similarly, $\tan$ is undefined at every odd multiple of $\pi/2$, since $\cos\left(\frac{(2n+1)\pi}{2}\right)=0$ for every integer $n$. Equivalently, its domain is $\R\setminus\left\{\frac{(2n+1)\pi}{2}:n\in\mathbb{Z}\right\}$. The range of $\tan$ is all of $\R$.

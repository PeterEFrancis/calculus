...
\end{itemize}
In this example, $\varepsilon = 1/10$ and $\delta = 1/30$. However, if we want to really prove that the limit is 7, we let $\varepsilon>0$ be arbitrary, and take $\delta = \varepsilon/3$. Then for any $x$ such that $|x-2|<\delta$,
...
Intuitively, a function is continuous if you can draw its graph without lifting up your pencil. We can use limits to write a more precise definition: a function $f$ is \textbf{continuous at $a$} if
$$\lim_{x\to a}f(x)=f(a).$$
This equation implicitly requires three things:
\begin{enumerate}
\item[(a)] $f$ is defined at $a$ (i.e. $f(a)$ makes sense),
\item[(b)] $\displaystyle\lim_{x\to a}f(x)$ exists, and
\item[(c)] $\displaystyle\lim_{x\to a}f(x)=f(a)$.
\end{enumerate}

A function is \textbf{continuous from the right at $a$} if $\displaystyle\lim_{x\to a^+}f(x)=f(a)$, and it is \textbf{continuous from the left at $a$} if $\displaystyle\lim_{x\to a^-}f(x)=f(a)$. At an included endpoint of an interval, continuity means continuity from the side lying in the interval.

A function is \textbf{continuous on an interval} if it is continuous at every interior number in the interval and has the appropriate one-sided continuity at each included endpoint.

\noindent\textbf{Definition:} If $f$ is defined near $a$ (on an open interval containing $a$, except possibly at $a$ itself) but is not continuous at $a$, then $f$ is \textbf{discontinuous at $a$}. The phrase ``except possibly at $a$'' is important: $f(a)$ need not be defined, so holes and vertical asymptotes at omitted interior $x$-values still count as discontinuities. At an included endpoint, failure of the appropriate one-sided condition likewise makes $f$ discontinuous there.

Here are three types of discontinuities that you may encounter:
\begin{enumerate}
\item $f$ has a \textbf{removable discontinuity} at $a$ if $\displaystyle\lim _{x \to a} f(x)$ exists and is a real number, but $f(a)$ is either undefined or unequal to this limit.
...
For example, $\frac{x^2-9}{x-3}$ has a removable discontinuity at $3$, while $\frac{1}{x}$ has an infinite discontinuity at $0$, even though neither function is defined at the indicated number.

Not every number outside the domain is a discontinuity: the condition that $f$ be defined near $a$ rules out unrelated numbers and missing domain endpoints.
...
You should believe that the following types of functions are continuous at every number in their domains:
\begin{itemize}
\item polynomials
...
\begin{tikzpicture}[
    ...
    choice/.style={draw, rounded corners, align=center, font=\footnotesize,
        minimum height=1cm, inner xsep=3pt, inner ysep=4pt}
]
\node[choice] (plug) at (0,2.2) {plug $\displaystyle a$ into $f$};

\node[choice, minimum width=1.6cm] (number) at (-4.57,0.5) {number};
\node[choice, text width=2.4cm] (overzero) at (-1.9,0.5)
    {$\displaystyle\frac{\text{non-zero number}}{0}$};
\node[choice, text width=2cm] (indeterminate) at (1.08,0.5)
    {indeterminate\\form};
\node[choice, text width=2.4cm] (undefined) at (4.06,0.5)
    {undefined for\\another reason};

\node (number-result) at (-4.57,-0.9) {$f(a)$};
\node (overzero-result) at (-1.9,-0.9) {DNE or $\displaystyle\pm\infty$};
\node[align=center] (indeterminate-result) at (1.08,-0.9) {More work\\needed};
\node[align=center] (undefined-result) at (4.06,-0.9) {More work\\needed};

\draw[->] (plug.220) -- (number.north);
\draw[->] (plug.250) -- (overzero.north);
\draw[->] (plug.290) -- (indeterminate.north);
\draw[->] (plug.320) -- (undefined.north);
\draw[->] (number.south) -- (number-result.north);
\draw[->] (overzero.south) -- (overzero-result.north);
\draw[->] (indeterminate.south) -- (indeterminate-result.north);
\draw[->] (undefined.south) -- (undefined-result.north);
\end{tikzpicture}
    }
...
They are called \textbf{indeterminate} because in this form, we cannot be sure of its value (or even if its value makes sense). Take $0^0$ for example: zero raised to any power should be 0, but anything raised to the power of $0$ should be 1... so which is it? 

When we get an indeterminate form after trying to evaluate $f(a)$, we need to do more work. There are several things you can try: \mar{Write out similar question for the other 6 indeterminate forms.}
...
Later, we will learn a trick called L'H\^opital's Rule to more easily compute limits with indeterminate forms. But first we need to learn about derivatives!


\subsection{Undefined for Another Reason}

Sometimes plugging $a$ into $f$ is undefined without producing a non-zero number over 0 or an indeterminate form. In this case, here are three things you can try:

\begin{itemize}
\item Use the Squeeze Theorem, especially when part of the function is bounded:
...
as well.
\end{thm}
For example, since $\left|\cos(1/x)\right|\leq 1$,
$$-|x|\leq x\cos(1/x)\leq |x|,$$
so the Squeeze Theorem gives $\displaystyle\lim_{x\to0}x\cos(1/x)=0$.

\item Make a change of variables to turn the limit into a more familiar one. Be sure to determine what the new variable approaches.

\item Find the limit of the inner expression first, then use the Composite Function Theorem if the outer function is continuous at that limit.
